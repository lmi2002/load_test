...
class TestUbApiDrorm:
    host = 'https://register.test.nais.gov.ua'
    path = 'https://register.test.nais.gov.ua/ubql'
    authorization = 'CERT2 5b31e429638f662e20c2d352'  # Авторизируемся и берем в заголовке Authorization
    ...
    serial = '363043803E9A341C04000000C700000084010000'

    def post_response(self, data):
        return requests.post(self.path, headers=self.headers, data=json.dumps(data))

    def get_response(self, payload, endpoint=None):
        if endpoint is None:
            requests.get(self.host, headers=self.headers, params=payload)
        else:
            requests.get(self.host + endpoint, headers=self.headers, params=payload)
    ...
    # @pytest.mark.skip
    def test_drorm_create_lmn(self):
        ...
        response_create_st = self.post_response(data_create_st).json()[0]
        ...
        response_generate_print_doc = self.post_response(data=data_generate_print_doc).json()[0]
        ...
        self.post_response(data_generate_pdf_doc)

        st_date_time = self.get_now_strftime("%Y-%m-%dT%H-2:%M:%S.000Z")
        ...
        self.post_response(data_sign_pdf_st_card)
        ...
        response_create_lmn = self.post_response(data_create_lmn).json()[0]
        ...
        self.post_response(data_select_lmn)
        ...
        response_generate_extract_vityag = self.post_response(data_generate_extract_vityag).json()[0]
        ...
        self.post_response(data_select_document_vityag)
        ...
        self.get_response(data_generated_document_vityag)

    # @pytest.mark.skip
    def test_search_pers_code(self):
        ...
        response_generate_extract_registry = self.post_response(data_generate_extract_registry).json()[0]
    ...
    # @pytest.mark.skip
    def test_search_comp_code(self):
        # Створення Заява про надання інформації з Державного реєстру обтяжень рухомого майна
        sbj_сode = '33261467'
        op_id = None
        data_create_st = [
